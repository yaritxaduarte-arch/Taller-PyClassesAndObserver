...
export function getRepositoryFromEnv() {
  ...
  if (!repository || !repository.includes("/")) {
    throw new Error("No se encontro GITHUB_REPOSITORY. Ejecuta este script desde GitHub Actions.");
  }
...
export function getTokenFromEnv() {
  ...
  if (!token) {
    throw new Error("No se encontro GITHUB_TOKEN. El workflow debe pasarlo desde secrets.GITHUB_TOKEN.");
  }
...
export class GitHubApi {
  ...
  async request(method, path, options = {}) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`GitHub API ${method} ${path} fallo con estado ${response.status}: ${message}`);
    }
